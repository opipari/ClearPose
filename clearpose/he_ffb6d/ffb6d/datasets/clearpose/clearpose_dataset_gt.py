    ...
    def get_item(self, item_name):
        ...
        if self.depth_type == "GT":
            ...
            with Image.open(os.path.join(self.root, scene_idx, set_idx, data_idx+'-depth.png')) as di:
                dpt_um = np.array(di)    
        elif self.depth_type == "depthcomplete_transcg_raw":
            with Image.open(os.path.join(self.root, scene_idx, set_idx, data_idx+'_TG_raw_pred.png')) as di:
                dpt_um = np.array(di)        
        elif self.depth_type == "depthcomplete_transcg_mask":
            with Image.open(os.path.join(self.root, scene_idx, set_idx, data_idx+'_TG_masked_pred.png')) as di:
                dpt_predict = np.array(di)
            with Image.open(os.path.join(self.root, scene_idx, set_idx, data_idx+'-depth.png')) as di:
                dpt_raw = np.array(di)      
            dpt_um = dpt_raw.copy()
            dpt_um[dpt_predict!=0] = dpt_predict[dpt_predict!=0]   
        elif self.depth_type == "depthcomplete_id_mask":
            with Image.open(os.path.join(self.root, scene_idx, set_idx, data_idx+'-ID-mask.png')) as di:
                dpt_um = np.array(di)    
