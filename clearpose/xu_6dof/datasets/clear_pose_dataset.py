...
class ClearPoseDataset(Dataset):
	def __init__(self, image_list="./data/images.csv", 
		...
		self.diameters = torch.stack(self.diameters)
	...
	def __getitem__(self, idx, r=True):
		...
		crops_masks = roi_align(masks.unsqueeze(1).type(torch.float32), [bx.unsqueeze(0) for bx in target['boxes']], (80,80), 1, 1)
		crops_geometry_per_image = torch.cat([crops_normal, crops_plane, crops_uvz], dim=1)

		poses = meta[4]
		poses = np.transpose(poses, (2,0,1))[ign_ids]
		target["poses"] = torch.from_numpy(poses).unsqueeze(0)
		target["quats"] = torch.stack([torch.from_numpy(R.from_matrix(rmat[:, :3]).as_quat()) for rmat in poses])[:,[3,0,1,2]].unsqueeze(0)
		target["trans_gt"] = torch.stack([torch.from_numpy(rmat[:, 3]) for rmat in poses]).unsqueeze(0)
		...
		target["diameter"] = torch.from_numpy(np.array(self.diameters[target["labels"]]))
		target["symmetric"] = torch.from_numpy(np.array([self.object_list[label][2]!='normal' for label in target["labels"][0].tolist()]))
