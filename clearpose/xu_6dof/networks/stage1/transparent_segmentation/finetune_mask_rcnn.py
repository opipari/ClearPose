...
from clearpose.xu_6dof.datasets.clearpose_transparent_segmentation_dataset import TransparentSegmentationDataset
...
def main(config={"num_classes": 63}, save_dir=os.path.join("experiments","xu_6dof","stage1","transparent_segmentation","models")):
	...
	model.load_state_dict(torch.load(os.path.join(save_dir,"mask_rcnn_3.pt"))["model_state_dict"], strict=False)
	...
	torch.save({'epoch': -1,
				...
				os.path.join(save_dir,"finetune","mask_rcnn_0.pt"))

	for epoch in range(num_epochs):
		train_one_epoch(model, optimizer, data_loader, device, epoch, lr_scheduler_=lr_scheduler, print_freq=100, logfile=logfile, stop_at=5000)
		# evaluate(model, data_loader_test, device=device)
		torch.save({'epoch': epoch,
				'model_state_dict': model.state_dict(),
				'optimizer_state_dict': optimizer.state_dict(),
				'scheduler_state_dict': lr_scheduler.state_dict()},
				os.path.join(save_dir,"finetune","mask_rcnn_"+str(epoch)+".pt"))
