...
def standard_test(root, train_sets):
	...
	for sett in train_sets:
		...
		for scene in scenes:
			img_id, test_summary_file = save_scene(scene, root_set, test_summary_file, img_id, sample_rate=21)
...
def occlusion_test(root):
	...
	for sett in ['set2', 'set3']:
		...
		for scene in scenes:
			img_id, test_summary_file = save_scene(scene, root_set, test_summary_file, img_id, sample_rate=38)
	test_summary_file.close()
...
def wou_test(root):# water opaque unseen background
	...
	for scene in scenes:
		img_id, test_summary_file = save_scene(scene, root_set, test_summary_file, img_id, sample_rate=3)
	test_summary_file.close()
...
def color_test(root):
	...
	for s in scene:
		...
		img_id, test_summary_file = save_scene(scene_nam, root_set, test_summary_file, img_id, sample_rate=2)
	test_summary_file.close()
...
def standard_train(root, train_sets):
	...
	for sett in sets:
		...
		for scene in scenes:
			img_id, train_summary_file = save_scene(scene, root_set, train_summary_file, img_id, sample_rate=1)
